...
The repo doubles as the on-host service folder under
`~/docker/attrakttv/` on [horst](https://horst.jemu.name), following the
one-folder-per-service Docker convention used for every other app on
that box. State (`./data/tRakt.db`) and secrets (`.env`) live next to
`docker-compose.yml`.
...
The image pins the in-container `attrakttv` user to UID/GID **1000** so
the bind-mounted `./data/` is writable when the host operator’s UID is
also 1000 (the Debian default and what horst uses). If your host UID
differs, build with explicit arg overrides:
...
The DESCRIPTION-only deps layer stays cached unless dependencies
changed, so code-only updates rebuild in seconds.
